...
export default function DropDown({handlePageName}) {
  return (
    <div className="">
          ...
          {navLinks.map((obj, id) => {
            return (
              <li key={id} className=''>
                ...
                <Link onClick={()=>handlePageName(obj.name)} to={obj.link} className='text-gray-200  text-sm  hover:font-medium hover:text-textPrimary hover:text-[15px] '>
                  {obj.name}
                </Link>
              </li>
            )
